...
public class PrintUtility {
    ...
    static void printBestSolution(Game game, GameSolver gameSolver) {
        ...
        String tempString =  " Best Solution #" + printSolutionCount;
        System.out.println(tempString );
//        System.out.println("<table bgcolor=#333333>");
        ...
        HashMap<Block, Blockfield> blockNameMapping = new HashMap<Block,Blockfield>();
            ...
            if (iterator.stepNumberToSolution > 0) {
                ...
                if (nextBoardBestSolution.stepNumberToSolution >= iterator.stepNumberToSolution) {
                    ...
                    if (!SIMPLE_SOLUTION) tempString += stepNumber + ": Board #" + iterator.idOfBoardExplored
                            + "->" + ((Blockfield)blockNameMapping.get(moveBestSolution.oldBlock)) + "(" +moveBestSolution.oldBlock.hashString() + ") " + moveBestSolution.moveType;
                    else tempString += stepNumber + ": " + ((Blockfield)blockNameMapping.get(moveBestSolution.oldBlock))  + " " + moveBestSolution.moveType;
                    tempString += "</td>";
        ...
        for (Block block : finalStep.blocks) {
            block.blockfield = blockNameMapping.get(block);
        }
        printBoardLayout(finalStep);
        System.out.println("</td></tr></table>");
        tempString =  " " + gameSolver.getClass().getSimpleName() + " End Best Solution #" + printSolutionCount ;
    ...
    public static void printTestMove(Board boardConfig, int i, Block oldBlock, MoveType moveType, Board nextBoardInTest) {
        ...
        System.out.println("#"+boardConfig.stepNumberToInitialNode+"."+i+"."+moveType+"->"+oldBlock.blockfield.blockType + " " + moveType+"->"+nextBoardInTest.stepNumberToInitialNode + "(" + PrintUtility.getBoardInfo(nextBoardInTest) + ")");
    }
...
//            tempString = "<tr>";
            for (int j = 0; j <= Board.MAXXPOS; j++) {
                ...
                for (int k = 0; k < boardConfig.blocks.length; k++) {
                    if (j >= boardConfig.blocks[k].xPos && j < boardConfig.blocks[k].xPos + boardConfig.blocks[k].blockfield.width
                            && i >= boardConfig.blocks[k].yPos && i < boardConfig.blocks[k].yPos + boardConfig.blocks[k].blockfield.height) {
                        isEmpty = false;
                        if (i == boardConfig.blocks[k].yPos && j == boardConfig.blocks[k].xPos) {
                            switch (boardConfig.blocks[k].blockfield.blockType) {
                                case SQUARE:
                                    tempString += "<td colspan=2 rowspan=2 width=70 height=70 align=center valign=cener bgcolor=#dddddd>" + boardConfig.blocks[k].blockfield.blockType + "</td>";
                                    break;
                                case HORIZONTAL:
                                    tempString += "<td colspan=2 width=70 height=35 align=center valign=cener bgcolor=#dddddd>" + boardConfig.blocks[k].blockfield.blockType+ "</td>";
                                    break;
                                case VERTICAL:
                                    tempString += "<td rowspan=2 width=35 height=70 align=center valign=cener bgcolor=#dddddd>" + boardConfig.blocks[k].blockfield.blockType + "<br>" + boardConfig.blocks[k].blockfield.blockType+ "</td>";
                                    break;
                                case SINGLE:
                                    tempString += "<td width=35 height=35 align=center valign=cener bgcolor=#dddddd>" + boardConfig.blocks[k].blockfield.blockType + "</td>";
                                    break;
                            }
                        }
